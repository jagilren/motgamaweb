...
RUN set -x; \
        ...
        ln -s http https \
        echo 'deb https://apt-archive.postgresql.org/pub/repos/apt stretch-pgdg-archive main' > etc/apt/sources.list.d/pgdg.list \
...
RUN pip3 install pyminizip==0.2.4
RUN pip3 install pyzipper
