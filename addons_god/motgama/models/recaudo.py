...
class MotgamaRecaudo(models.Model):
    ...
    def anular(self, **kwargs):
        self.ensure_one()
        if not self.env.ref('motgama.motgama_anular_recaudo') in self.env.user.permisos:
            raise Warning('No tiene permitido anular recaudos, contacte al administrador')
        horaCierre = self.env['motgama.parametros'].search([('codigo','=','HORACIERRE')],limit=1)
        horaTz = datetime.strptime(horaCierre.valor,'%H:%M').time()
        fechaFactura = self.factura['create_date']
        fechaFacturaHoraActual = fechaFactura - timedelta(hours=5)
        _logger.info("fecha factura actual")
        _logger.info(fechaFacturaHoraActual)
        horaFinal = datetime.strptime("23:59:59", "%H:%M:%S").time()
        horaActual = datetime.now() - timedelta(hours=5)
        horaActualFormateada = horaActual.strftime("%H:%M:%S")
        horaActualFormateada2 = datetime.strptime(horaActualFormateada, "%H:%M:%S").time()
        diaHoraActual = date.today()
        horaInicio = datetime.strptime("00:00:00", "%X").time()

        if (horaTz < horaActualFormateada2) and (horaActualFormateada2 <  horaFinal):
            horaComp = datetime.strptime("18:59:59", "%H:%M:%S").time()
            if(horaActualFormateada2 > horaComp):
                diaHorasActual = diaHoraActual - timedelta(days=1)
                fechaEnString = str(diaHorasActual) + " " + str(horaTz)
            else:
                fechaEnString = str(diaHoraActual) + " " + str(horaTz)
            fechaUltimoCierre = datetime.strptime(fechaEnString, "%Y-%m-%d %H:%M:%S")
            _logger.info("Fecha de cierre quedo 1 ")
            _logger.info(fechaUltimoCierre)
        elif (horaInicio < horaActualFormateada2) and (horaActualFormateada2 < horaTz):
            diaHorasActual = diaHoraActual - timedelta(days=1)
            fechaEnString = str(diaHorasActual) + " " + str(horaCierre)
            fechaUltimoCierre = datetime.strptime(fechaEnString, "%Y-%m-%d %H:%M:%S")
            _logger.info("Fecha de cierre quedo 2 ")
            _logger.info(fechaUltimoCierre)
       
        if fechaFacturaHoraActual < fechaUltimoCierre:
            raise Warning('Esta factura fue de un cierre anterior no es permitido modificar la forma de pago')
