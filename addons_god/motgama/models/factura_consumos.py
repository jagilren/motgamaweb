...
class MotgamaFacturaConsumos(models.Model):
    ...
    def val_consumos(self):
        val_line = {}
        for line in self.consumo_ids:
            if line.producto_id in val_line:
                val_line[line.producto_id]+=line.cantidad
            else:
                val_line[line.producto_id]=line.cantidad

        for producto in val_line:
            if val_line[producto]<0:
                raise Warning('El producto '+ producto.name + 'está con valores negativos')
        return True
