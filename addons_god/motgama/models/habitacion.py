...
class MotgamaFlujoHabitacion(models.Model):#adicionada por Gabriel sep 10
    ...
    def button_continuar(self):
        self.ensure_one()
        if not self.env.ref('motgama.motgama_rehabilita_habitacion') in self.env.user.permisos:
            raise Warning('No tiene permitido rehabilitar habitaciones, contacte al administrador')
        movimiento = self.ultmovimiento
    ...
    def revertir_abono(self):
        self.ensure_one()
        if not self.env.ref('motgama.motgama_devuelve_abono') in self.env.user.permisos:
            raise Warning('No tiene permitido devolver abonos, contacte al administrador')
        return {
