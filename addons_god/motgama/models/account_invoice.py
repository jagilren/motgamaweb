...
class Invoice(models.Model):
    ...
    def _prepare_refund(self, invoice, date_invoice=None, date=None, description=None, journal_id=None):
        if not self.env.ref('motgama.motgama_rectificativa') in self.env.user.permisos:
            raise Warning('No tiene permitido agregar factura rectificativa, contacte al administrador')
    ...
    def copy(self,default=None):
        if not self.env.ref('motgama.motgama_duplicar_factura') in self.env.user.permisos:
            raise Warning('No tiene permitido duplicar factura, contacte al administrador')
        return super().copy(default)

    @api.multi
    def unlink(self):
        if not self.env.ref('motgama.motgama_eliminar_factura') in self.env.user.permisos:
            raise Warning('No tiene permitido eliminar factura, contacte al administrador')
        return super().unlink()
